...
def _rows(module: str):
    ...
    assert path.exists(), f"missing source coverage for priority module: {module}"
    with path.open(newline="", encoding="utf-8") as handle:
        yield from csv.DictReader(handle)


def test_priority_source_coverage_is_fail_closed():
...
def test_priority_source_coverage_has_explicit_blocker_for_unpublished_sources():
