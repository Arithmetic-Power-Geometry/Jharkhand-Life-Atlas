...
MODULE = ROOT / "modules" / "weather_climate"
...
def test_module16_is_fail_closed_and_dependency_safe():
    module = _yaml("module.yaml")
    assert module["id"] == "weather_climate"
    ...
    assert "climatological-normal" in module["engineering"]["climate_policy"].lower()
    assert "do not publish" in module["engineering"]["trend_policy"].lower()


def test_module16_publication_gate_starts_closed():
...
def test_module16_sources_do_not_claim_unacquired_payloads():
...
def test_module16_forbids_false_precision_and_missing_as_zero():
    module = _yaml("module.yaml")
    ...
    assert "station/grid" in gate_text
