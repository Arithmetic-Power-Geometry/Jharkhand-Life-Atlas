...
hero("Modules", "JLA discovers valid module folders dynamically. The next module can be added without redesigning the application.", ["Folder contract", "Auto-discovery", "Independent provenance"])
...
for m in mods:
    with st.expander(f"{m.get('name')} · v{m.get('version')} · {m.get('status')}", expanded=m.get('id')=='core_geography'):
        ...
        if coverage:
            st.markdown('**Coverage contract**')
            for k,v in coverage.items(): st.write(f'- **{k.replace("_"," ").title()}:** {v}')
        data=module_indicators(m.get('_path',''))
        if data is not None:
            if data.height==0: st.info('Module contract is active; no indicator observations have been published yet.')
