"""Acquire the exact official Pakur HMIS November 2019-20 resource fail-closed.

This path starts from the verified authoritative data.gov.in resource page, follows
only explicit OGD-hosted download links, rejects HTML masquerading as data, hashes any
admitted CSV, and verifies the source-advertised schema before recording acquisition.
The source remains sub-district aggregate evidence: it must never be allocated to
villages, facilities or persons, and publication remains closed pending indicator,
period and geography validation.
"""
...
RESOURCE_PAGE = "https://www.data.gov.in/resource/item-wise-report-pakur-jharkhand-november-2019-20"
SOURCE_ID = "OGD_HMIS_PAKUR_2019_20_NOVEMBER"
...
def run(output_dir: Path) -> dict:
    ...
    report = {
        "contract": "JLA_HEALTH_HMIS_PAKUR_OFFICIAL_PAYLOAD_ACQUISITION_V1",
        "source_id": SOURCE_ID,
        "resource_page": RESOURCE_PAGE,
        "retrieved_at_utc": datetime.now(timezone.utc).isoformat(),
        "authoritative_resource_metadata": {
            "published_on": "2020-11-13",
            "updated_on": "2020-11-13",
            "reference_period": "2019-20_november",
            "status_as_on": "2020-09-25T14:09:00+05:30",
            "granularity": "Monthly",
            "advertised_format": "csv",
            "advertised_file_size": "54 KB",
            "resource_api_state": "does_not_exist",
            "expected_columns": EXPECTED_COLUMNS,
            "provisional_figures": True,
            "aggregation_semantics": "Total [(A+B) OR (C+D)] = [(Public Facilities + Private Facilities) OR (Rural Facilities + Urban Facilities)]",
        },
        ...
        "rules": [
            ...
            "subdistrict_aggregates_never_allocated_to_villages_facilities_or_persons",
            "provisional_source_status_must_be_preserved",
            "aggregation_semantics_must_be_preserved",
    ...
    try:
        ...
        (output_dir / "hmis_pakur_2019_20_november_acquisition.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
        return report
        ...
        for url in links:
            try:
                ...
                if response.status_code == 200 and attempt.get("accepted"):
                    payload = output_dir / "hmis_pakur_2019_20_november_raw.csv"
    ...
    (output_dir / "hmis_pakur_2019_20_november_acquisition.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    return report


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", type=Path, default=Path("build/health_hmis_pakur_2019_20_november"))
